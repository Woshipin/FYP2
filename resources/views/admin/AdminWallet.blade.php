...
<br><br><br>
    ...
    <div class="container">
        ...
        <h3 style="color: white">Admin Wallet</h3>

        @if ($adminwallets)
            <div class="mt-4 d-flex flex-column rounded-border grey-color py-4 px-3">
                <span class="account-label">Admin Account Balance</span>
                <span class="balance">RM{{ $totalBalance ?? '0.00' }}</span>
            </div>

            <div class="mt-2 d-flex flex-row justify-content-between rounded-border yellow-color px-3 py-4">
                <div class="d-flex flex-column">
                    <span class="currency-name">Tax</span>
                </div>
                <div class="d-flex flex-column align-items-end">
                    <span>RM{{ $totalTax ?? '0.00' }}</span>
                </div>
            </div>

            <div class="mt-2 d-flex flex-row justify-content-between rounded-border plain-color px-3 py-4">
                <div class="d-flex flex-column">
                    <span class="currency-name">User Deposit</span>
                </div>
                <div class="d-flex flex-column align-items-end">
                    <span>RM{{ $totalUserDeposit ?? '0.00' }}</span>
                </div>
            </div>
        @else
            <p>Admin Wallet information is not available.</p>
        @endif
