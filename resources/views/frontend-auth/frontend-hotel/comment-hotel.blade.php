...
@section('frontend-section')
    ...
    <style>
        ...
        * {
        ...
        .hotel-comment-post-button:hover {
        ...
        .hotel-comment-author-info h3 {
            font-size: 1rem;
            /* 增加作者名字字体大小 */
            font-weight: 600;
        }

        .hotel-comment-timestamp {
            font-size: 0.875rem;
            /* 增加时间戳字体大小 */
            color: var(--secondary-color);
        }

        .hotel-comment-content {
            margin: 0.5rem 0;
            font-size: 1rem;
            /* 增加评论内容字体大小 */
        }

        .hotel-comment-reply-button {
            background: none;
            border: none;
            color: var(--secondary-color);
            font-size: 0.875rem;
        ...
        .hotel-comment-reply-button:hover {
            background-color: var(--background-color);
        }

        .hotel-comment-replies {
            margin-left: 3rem;
            margin-top: 1rem;
        }
    ...
    <div class="hotel-comment-container">
        <div class="hotel-comment-section">
            <h2>Comments</h2>

            <!-- New Comment Form -->
            <div class="hotel-comment-form">
                ...
                <form action="{{ route('addHotelComment', ['id' => $hotel->id]) }}" id="comment-form" method="post">
                    ...
                    <button class="hotel-comment-post-button" type="submit">Post Comment</button>
                </form>
            </div>
                ...
                @if ($comments->count() > 0)
                    @foreach ($comments as $comment)
                        <div class="hotel-comment">
                            <div class="hotel-comment-header">
                                <div class="hotel-comment-author">
                                    <img src="https://via.placeholder.com/40" alt="{{ $comment->user_name }}"
                                        class="hotel-comment-avatar">
                                    <div class="hotel-comment-author-info">
                                        <h3>{{ $comment->user_name }}</h3>
                                        <span class="hotel-comment-timestamp">{{ $comment->created_at }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="hotel-comment-content">
                                <p>{{ $comment->comment }}</p>
                            </div>
                            <button class="hotel-comment-reply-button reply" cid="{{ $comment->id }}"
                                name_a="{{ Auth::user()->name }}" token="{{ csrf_token() }}"
                                data-parent-id="{{ $comment->id }}" data-parent-type="comment">Reply</button>
                            @if ($comment->user_id == Auth::id())
                                <button class="hotel-comment-reply-button delete-comment"
                                    data-id="{{ $comment->id }}">Delete</button>
                            @endif

                            <div class="hotel-comment-replies reply-form">
                                @if ($comment->replies && $comment->replies->count() > 0)
                                    @foreach ($comment->replies as $reply)
                                        <div class="hotel-comment hotel-comment-reply">
                                            <div class="hotel-comment-header">
                                                <div class="hotel-comment-author">
                                                    ...
                                                    <div class="hotel-comment-author-info">
                                                        <h3>{{ $reply->name }}</h3>
                                                        <span class="hotel-comment-timestamp">Replied ->
                                            ...
                                            <button class="hotel-comment-reply-button reply-to-reply"
                                                rname="{{ Auth::user()->name }}" rid="{{ $reply->id }}"
                                                token="{{ csrf_token() }}" data-parent-id="{{ $reply->id }}"
                                                data-parent-type="reply">Reply</button>
                                            @if ($reply->user_id == Auth::id())
                                                <button class="hotel-comment-reply-button delete-reply"
                                                    data-id="{{ $reply->id }}">Delete</button>
                                            @endif

                                            <div class="hotel-comment-replies reply-to-reply-form">
                                                    ...
                                                    @foreach ($reply->repliesToReplies as $replyToReply)
                                                        <div class="hotel-comment hotel-comment-reply">
                                                            ...
                                                            @if ($replyToReply->user_id == Auth::id())
                                                                <button
                                                                    class="hotel-comment-reply-button delete-reply-to-reply"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".hotel-comment-list").on("click", ".reply", function() {
                ...
                appendReplyForm(well, "{{ route('replyhotelcomment') }}", cid, name, token, parentId,
                    parentType);
            });

            $(".hotel-comment-list").on("click", ".delete-comment", function(e) {
                e.preventDefault();
                var commentId = $(this).data('id');
                var commentElement = $(this).closest('.hotel-comment');

                if (confirm("Are you sure you want to delete this comment?")) {
                    $.ajax({
                        url: "{{ route('deleteHotelComment', ['id' => ':id']) }}".replace(':id',
                            commentId),
                        type: 'DELETE',
            ...
            $(".hotel-comment-list").on("click", ".delete-reply", function(e) {
                e.preventDefault();
                var replyId = $(this).data('id');
                var replyElement = $(this).closest('.hotel-comment-reply');

                if (confirm("Are you sure you want to delete this reply?")) {
                    $.ajax({
                        url: "{{ route('deletereplyhotelcomment', ['id' => ':id']) }}".replace(
                            ':id', replyId),
                        type: 'DELETE',
                        success: function(result) {
                            if (result.success) {
            ...
            $(".hotel-comment-list").on("click", ".delete-reply-to-reply", function(e) {
                e.preventDefault();
                var replyToReplyId = $(this).data('id');
                var replyToReplyElement = $(this).closest('.hotel-comment-reply');

                if (confirm("Are you sure you want to delete this reply?")) {
                    $.ajax({
                        url: "{{ route('deleteReplyToReplyHotelComment', ['id' => ':id']) }}"
            ...
            $(".hotel-comment-list").on("click", ".reply-to-reply", function() {
                ...
                var parentId = $(this).data('parent-id');
                var parentType = $(this).data('parent-type');
                appendReplyToReplyForm(well, "{{ route('storeReplyToReplyHotel') }}", rid, rname, token,
                    parentId, parentType);
            });
        });
    </script>
