...
@section('frontend-section')
    ...
    <div class="container">
        ...
        <div class="restaurant-header">
            <div>
                <h1 class="restaurant-title" id="detail">{{ $restaurants->name }}</h1>
                <div class="restaurant-location">
                    <i class="fas fa-map-marker-alt"></i>
                    <span id="detail">{{ $restaurants->address }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="content-grid">
                ...
                <div class="image-gallery">
                    @if ($restaurants->images->isNotEmpty())
                        ...
                        <div class="main-image-container">
                            <img src="{{ asset('images/' . $restaurants->images->first()->image) }}" alt="Restaurant view"
                                class="main-image">
                        </div>
                        <!-- 缩略图网格 -->
                        <div class="thumbnail-grid">
                            @foreach ($restaurants->images->slice(1, 10) as $image)
                                <div class="thumbnail">
                                    <img src="{{ asset('images/' . $image->image) }}" alt="Restaurant view"
                                        onclick="show360Image('{{ asset('images/' . $image->image) }}')">
                                    <!-- 如果是第10张图片，显示 View More -->
                                    @if ($loop->index == 9 && $restaurants->images->count() > 10)
            ...
            <div class="info-column">
                ...
                <div class="rating-modal">
                    ...
                    <form action="{{ route('restaurantratings') }}" method="POST" class="rating-form">
                        @csrf
        ...
        <div class="columns">
            <!-- Comments Column -->
            <div class="column">
                <a href="{{ route('restaurants.comment', ['id' => $restaurants->id]) }}">
            ...
            {{-- <div class="column facility-column" id="facility-column">
                ...
                <div class="column-content">
                    ...
                    <p>Restaurant facilities and amenities details</p>
                </div>
            </div> --}}
        ...
        <div class="map-section">
            <iframe src="{{ $restaurants->map }}" class="map-frame" allowfullscreen></iframe>
        </div>
        ...
        <div id="imageModal" class="modal">
            <div class="modal-content">
                ...
                <div class="modal-thumbnail-grid">
                    @foreach ($restaurants->images->slice(10) as $image)
        ...
        <div id="community-modal" class="community-modal">
            <div class="community-modal-content">
                <div class="community-modal-header">
                    <h2>{{ $restaurants->name }}<span class="community-stars">
                            @for ($i = 1; $i <= 5; $i++)
                ...
                <div class="community-modal-body">
                    ...
                    <div class="community-info-container">
                        <p class="community-address" id="detail"><span
                                class="community-icon">📍</span>{{ $restaurants->address }}</p>
                        <div class="community-rating">
                            <span class="community-rating-score">{{ $averageRating ?? '0' }}</span>
                            {{-- <span class="community-rating-text">{{ $restaurants->location }}</span> --}}
                                        ...
                                        @if ($community->category == $category->name)
                                            <div class="community-station-card"
                                                ...
                                                <div class="community-view-icon">
                                                    ...
                                                    <a href="{{ route('restaurant.community.detail', ['id' => $community->id]) }}" class="community-tooltip">View more</a>
                                                </div>
                                            </div>
                                        @endif
                                    @endforeach
        ...
        {{-- <div id="facility-modal" class="facility-modal">
            <div class="facility-modal-content">
                <div class="card-header">
                    <h2 class="card-title">{{ $restaurants->name }} Facility</h2>
                    <span class="facility-close">&times;</span>
                </div>
                <div class="card-content">
                    <div class="facilities-grid">
                        @foreach ($restaurants->facilities as $facility)
    ...
    {{-- <script>
        document.addEventListener("DOMContentLoaded", function() {
            const latitude = {{ $restaurant->latitude ?? 1.3521 }};
            const longitude = {{ $restaurant->longitude ?? 103.8198 }};
            ...
            const initMap = () => {
                ...
                const restaurantMarker = L.marker([latitude, longitude]).addTo(map);
                restaurantMarker.bindPopup(`<b>${restaurantName}</b><br>Location: ${latitude}, ${longitude}`)
                    .openPopup();
